...
//
// The server hands us a per-session key (`enc_key`, standard base64) at admin-login. When we send
// `X-Enc: 1` on an /admin request, the request body must be the url-safe base64 (no padding) of
...
// It keeps console JSON out of the Network tab as readable text; authorization is always
// enforced server-side, so an operator who decrypts their own traffic still cannot reach
// anything their admin token isn't scoped for.
